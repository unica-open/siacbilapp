...
import it.csi.siac.siacfin2app.frontend.ui.model.provvisoriocassa.AssociaQuoteEntrataAProvvisorioDiCassaModel;
...
@PutModelInSession
public class AssociaQuoteEntrataAProvvisorioDiCassaAction extends AssociaQuoteAProvvisorioDiCassaAction<AssociaQuoteEntrataAProvvisorioDiCassaModel> {

	/** Per la serializzazione */
	private static final long serialVersionUID = 3990403464564764568L;
	...
	public String completeStep2() {
		...
		AssociaProvvisorioQuoteEntrata req = model.creaRequestAssociaProvvisorioQuoteEntrata();
		logServiceRequest(req);
		AssociaProvvisorioQuoteEntrataResponse res = documentoEntrataService.associaProvvisorioQuoteEntrata(req);
	...
	private void aggiornaQuoteAssociate(List<SubdocumentoEntrata> listaQuoteAssociate) {
		List<SubdocumentoEntrata> listaQuote = sessionHandler.getParametro(BilSessionParameter.RISULTATI_RICERCA_QUOTE_ENTRATA_PER_PROVVISORIO);
		Map<Integer, SubdocumentoEntrata> map = new HashMap<Integer, SubdocumentoEntrata>();
		
		for (SubdocumentoEntrata q : listaQuote) {
			map.put(q.getUid(), q);
		}
		for (SubdocumentoEntrata q : listaQuoteAssociate) {
			SubdocumentoEntrata se = map.get(q.getUid());
			if(se != null) {
				se.setProvvisorioCassa(q.getProvvisorioCassa());
			}
		}
		sessionHandler.setParametro(BilSessionParameter.RISULTATI_RICERCA_QUOTE_ENTRATA_PER_PROVVISORIO, listaQuote);
	}
	...
	protected void checkImportoDaRegolarizzare() {
		...
		BigDecimal totaleQuote = model.getTotaleSubdocumentiEntrata();
	...
	private int caricaQuote() throws WebServiceInvocationFailureException {
		...
		RicercaQuotePerProvvisorioEntrata req = model.creaRequestRicercaQuotePerProvvisorioEntrata();
		logServiceRequest(req);
		RicercaQuotePerProvvisorioEntrataResponse res = documentoEntrataService.ricercaQuotePerProvvisorioEntrata(req);
		logServiceResponse(res);
		...
		sessionHandler.setParametro(BilSessionParameter.RISULTATI_RICERCA_QUOTE_ENTRATA_PER_PROVVISORIO, res.getListaSubdocumenti());
		sessionHandler.setParametroXmlType(BilSessionParameter.REQUEST_RICERCA_QUOTE_ENTRATA_PER_PROVVISORIO, req);
		
		log.debug(methodName, "Totale quote di entrata: " + res.getTotaleElementi());
