...
import it.csi.siac.siacfin2app.frontend.ui.model.documentoiva.RicercaDocumentoIvaEntrataModel;
...
/**
 * Classe di action per la ricerca del Documento IVA di Entrata
 * 
...
public class RicercaDocumentoIvaEntrataAction extends GenericDocumentoIvaEntrataAction<RicercaDocumentoIvaEntrataModel> {
	...
	public String ricerca() {
		...
		RicercaSinteticaSubdocumentoIvaEntrata request = model.creaRequestRicercaSinteticaSubdocumentoIva();
		logServiceRequest(request);
		RicercaSinteticaSubdocumentoIvaEntrataResponse response = documentoIvaEntrataService.ricercaSinteticaSubdocumentoIvaEntrata(request);
		...
		int totaleElementi = response.getListaSubdocumentoIvaEntrata().getTotaleElementi();
		...
		sessionHandler.setParametro(BilSessionParameter.RISULTATI_RICERCA_DOCUMENTI_IVA_SPESA, response.getListaSubdocumentoIvaEntrata());
	...
	public void validateRicerca() {
		DocumentoEntrata d = model.getDocumento();
	...
	private void caricaListaTipoDocumento() {
		...
		if(listaInSessione == null) {
			RicercaTipoDocumento request = model.creaRequestRicercaTipoDocumento(TipoFamigliaDocumento.ENTRATA, null, null);
			logServiceRequest(request);
