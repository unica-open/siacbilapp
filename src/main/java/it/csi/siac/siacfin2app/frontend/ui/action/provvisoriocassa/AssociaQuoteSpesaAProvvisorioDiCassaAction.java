...
public class AssociaQuoteSpesaAProvvisorioDiCassaAction extends AssociaQuoteAProvvisorioDiCassaAction<AssociaQuoteSpesaAProvvisorioDiCassaModel> {
	...
	public String completeStep2() {
		...
		if(res.hasErrori()) {
			...
			log.info(methodName, createErrorInServiceInvocationString(AssociaProvvisorioQuoteSpesa.class, res));
			addErrori(res);
			return INPUT;
		}
	...
	private int caricaQuote() throws WebServiceInvocationFailureException {
		...
		if(res.hasErrori()) {
			//si sono verificati degli errori: esco.
			String errorString = createErrorInServiceInvocationString(RicercaQuotePerProvvisorioSpesa.class, res);
