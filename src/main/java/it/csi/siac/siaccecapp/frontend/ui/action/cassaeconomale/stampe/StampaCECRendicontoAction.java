...
public class StampaCECRendicontoAction extends GenericStampaCECAction<StampaCECRendicontoModel> {
	...
	public String enterStep4() {
		...
		if (res.hasErrori()) {
			log.info(methodName, createErrorInServiceInvocationString(StampaRendicontoCassa.class, res));
			addErrori(res);
			return INPUT + resultSuffix();
		}
	...
	private void validateSoggetto() {
		...
		if(soggetto == null || !model.getSoggetto().getCodiceSoggetto().equals(soggetto.getCodiceSoggetto())) {
			...
			if(res.hasErrori()) {
				//si sono verificati degli errori: esco.
				addErrori(res);
				throw new ParamValidationException(createErrorInServiceInvocationString(RicercaSoggettoPerChiave.class, res));
			}
	...
	public String caricaUltimaDefinitiva() {
		...
		if (response.hasErrori()) {
			log.info(methodName, createErrorInServiceInvocationString(RicercaUltimoRendicontoCassaStampato.class, response));
			addErrori(response);
			return SUCCESS;
		}
