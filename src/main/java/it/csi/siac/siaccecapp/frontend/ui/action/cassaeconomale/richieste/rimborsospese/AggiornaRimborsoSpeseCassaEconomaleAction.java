...
/**
 * Classe di action per l'aggiornamento del rimborso spese.
 * 
...
public class AggiornaRimborsoSpeseCassaEconomaleAction extends BaseInserisciAggiornaRimborsoSpeseCassaEconomaleAction<AggiornaRimborsoSpeseCassaEconomaleModel> {
	...
	public String completeStep2() {
		...
		logServiceRequest(request);
		AggiornaRichiestaEconomaleResponse response = richiestaEconomaleService.aggiornaRichiestaEconomaleRimborsoSpese(request);
		logServiceResponse(response);
	...
	protected AzioniConsentite[] retrieveAzioniConsentite() {
		return new AzioniConsentite[] {AzioniConsentite.CASSA_ECONOMALE_RIMBORSO_SPESE_AGGIORNA, AzioniConsentite.CASSA_ECONOMALE_RIMBORSO_SPESE_ABILITA};
	}
}
