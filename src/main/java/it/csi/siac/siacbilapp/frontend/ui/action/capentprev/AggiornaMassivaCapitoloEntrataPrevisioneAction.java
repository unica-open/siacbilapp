...
/**
 * Classe di Action per la gestione massiva dell'aggiornamento del Capitolo di Entrata Previsione.
 * 
...
public class AggiornaMassivaCapitoloEntrataPrevisioneAction extends CapitoloEntrataAction<AggiornaMassivaCapitoloEntrataPrevisioneModel>{
	
	/** Per la serializzazione */
	private static final long serialVersionUID = 8645620311189470110L;
	...
	public String execute() throws Exception {
		...
		RicercaDettaglioMassivaCapitoloEntrataPrevisione requestRicercaDettaglio = model.creaRequestRicercaDettaglioMassivaCapitoloEntrataPrevisione();
		logServiceRequest(requestRicercaDettaglio);
		
		log.debug(methodName, "Richiamo il WebService di ricerca dettaglio massiva");
		RicercaDettaglioMassivaCapitoloEntrataPrevisioneResponse responseRicercaDettaglio = capitoloEntrataPrevisioneService.ricercaDettaglioMassivaCapitoloEntrataPrevisione(requestRicercaDettaglio);
		...
		ControllaClassificatoriModificabiliCapitoloResponse responseClassificatoriModificabili = ottieniResponseControllaClassificatoriModificabiliCapitolo(model.getCapitoloEntrataPrevisione());
		ControllaAttributiModificabiliCapitoloResponse responseAttributiModificabili = ottieniResponseControllaAttributiModificabiliCapitolo(model.getCapitoloEntrataPrevisione());
	...
	public String aggiornaCDU() {
		...
		log.debug(methodName, "Creazione della request");
		AggiornaMassivoCapitoloDiEntrataPrevisione request = model.creaRequestAggiornaMassivoCapitoloDiEntrataPrevisione(classificatoreAggiornamento);
		logServiceRequest(request);
		
		log.debug(methodName,"Richiamo il WebService di aggiornamento massivo");
		AggiornaMassivoCapitoloDiEntrataPrevisioneResponse response = capitoloEntrataPrevisioneService.aggiornaMassivoCapitoloDiEntrataPrevisione(request);
	 ...
	 */
	private boolean controllaDatiCapitoli(RicercaDettaglioMassivaCapitoloEntrataPrevisioneResponse response) {
		boolean almenoUnCapitoloInStatoValido = false;
		for(CapitoloEntrataPrevisione capitolo : response.getCapitoloMassivaEntrataPrevisione().getElencoCapitoli()) {
	 ...
	 */
	private boolean valutaVariazioniCapitolo(CapitoloEntrataPrevisione capitolo) {
