...
/**
 * Classe per il caricamento <em>AJAX</em> del SIOPE di Spesa.
 * 
...
public class SiopeSpesaAjaxAction extends GenericClassificatoriAjaxAction<LeggiTreeSiope, LeggiTreeSiopeResponse> {
	
	/** Per la serializzazione */
	private static final long serialVersionUID = -4410910250503539069L;
	...
	protected void impostaPerInjezioneInVariazione(LeggiTreeSiopeResponse response) {
		injettaInVariazione(TipologiaClassificatore.SIOPE_SPESA, response.getTreeSiopeSpesa());
	}

	@Override
	protected void injettaResponseNelModel(LeggiTreeSiopeResponse response) {
		model.setListaElementoCodifica(ElementoCodificaFactory.getInstances(response.getTreeSiopeSpesa()));
	}
	...
	protected void sortDeiRisultati(LeggiTreeSiopeResponse response) {
		...
		try {
			ComparatorUtils.sortDeepByCodice(response.getTreeSiopeSpesa());
		} catch (Exception e) {
