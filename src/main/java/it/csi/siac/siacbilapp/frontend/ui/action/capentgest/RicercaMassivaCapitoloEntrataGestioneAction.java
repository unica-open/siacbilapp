...
import it.csi.siac.siacbilapp.frontend.ui.model.capentgest.RicercaMassivaCapitoloEntrataGestioneModel;
...
/**
 * Action per la gestione della Ricerca Massiva per il Capitolo di Entrata Gestione.
 * 
...
public class RicercaMassivaCapitoloEntrataGestioneAction 
		extends CapitoloEntrataAction<RicercaMassivaCapitoloEntrataGestioneModel> {
	...
	public String execute() throws Exception {
		...
		ControllaClassificatoriModificabiliCapitoloResponse responseClassificatoriModificabili = ottieniResponseControllaClassificatoriModificabiliCapitolo(model.getCapitoloEntrataGestione(), TipoCapitolo.CAPITOLO_ENTRATA_GESTIONE);
	...
	public String ricercaConOperazioniCDU() {
		...
		RicercaSinteticaMassivaCapitoloEntrataGestione request = model.creaRequestRicercaSinteticaMassivaCapitoloEntrataGestione();
		logServiceRequest(request);
		
		// Richiama il servizio di ricercaSinteticaCapitoloEntrataGestione()
		log.debug(methodName, "Richiamo il WebService di Ricerca Sintetica Massiva");
		
		RicercaSinteticaMassivaCapitoloEntrataGestioneResponse response = capitoloEntrataGestioneService.ricercaSinteticaMassivaCapitoloEntrataGestione(request);
	...
	public String ricercaVariazioneCodifiche() {
		...
		checkNotNull(model.getCapitoloEntrataGestione().getNumeroCapitolo(), "numero capitolo");
		checkNotNull(model.getCapitoloEntrataGestione().getNumeroArticolo(), "numero articolo");
		checkNotNull(model.getCapitoloEntrataGestione().getTipoCapitolo(), "tipo capitolo");
		...
		if(response.getCapitoloMassivaEntrataGestione() == null) {
	...
	public String ricercaDettaglio() {
		...
		checkNotNull(model.getCapitoloEntrataGestione().getAnnoCapitolo(), "Anno capitolo");
		checkNotNull(model.getCapitoloEntrataGestione().getNumeroCapitolo(), "Numero capitolo");
		checkNotNull(model.getCapitoloEntrataGestione().getNumeroArticolo(), "Numero articolo");
		if(hasErrori()) {
		...
		if(response.hasErrori()) {
			...
			addErrori(response);
			return SUCCESS;
		} else if(response.getCapitoloMassivaEntrataGestione() == null) {
		...
		log.debug(methodName, "Ricerca effettuata con successo");
		model.setCapitoloMassivaEntrataGestione(response.getCapitoloMassivaEntrataGestione());
		model.setListaCapitoloVariazione(ElementoCapitoloVariazioneFactory.getInstances(response.getCapitoloMassivaEntrataGestione().getElencoCapitoli(), Boolean.TRUE));
		sessionHandler.setParametro(BilSessionParameter.RISULTATI_RICERCA_MASSIVA_COME_DATO_AGGIUNTIVO, response.getCapitoloMassivaEntrataGestione().getElencoCapitoli());
		
		// Injetto il massimo valore della UEB
		Integer maxUEB = ComparatorUtils.getMaxUEB(response.getCapitoloMassivaEntrataGestione().getElencoCapitoli());
