...
/**
 * Classe di Action per la gestione massiva dell'aggiornamento del Capitolo di Uscita Previsione.
 * 
...
public class AggiornaMassivaCapitoloUscitaPrevisioneAction extends CapitoloUscitaAction<AggiornaMassivaCapitoloUscitaPrevisioneModel>{
	
	/** Per la serializzazione */
	private static final long serialVersionUID = -4959761326219391815L;
	...
	public String execute() throws Exception {
		...
		RicercaDettaglioMassivaCapitoloUscitaPrevisione requestRicercaDettaglio = model.creaRequestRicercaDettaglioMassivaCapitoloUscitaPrevisione();
		logServiceRequest(requestRicercaDettaglio);
		
		log.debug(methodName, "Richiamo il WebService di ricerca dettaglio massiva");
		RicercaDettaglioMassivaCapitoloUscitaPrevisioneResponse responseRicercaDettaglio = capitoloUscitaPrevisioneService.ricercaDettaglioMassivaCapitoloUscitaPrevisione(requestRicercaDettaglio);
	...
	public String aggiornaCDU() {
		...
		log.debug(methodName, "Creazione della request");
		AggiornaMassivoCapitoloDiUscitaPrevisione request = model.creaRequestAggiornaMassivoCapitoloDiUscitaPrevisione(classificatoreAggiornamento);
		logServiceRequest(request);
		
		log.debug(methodName,"Richiamo il WebService di aggiornamento massivo");
		AggiornaMassivoCapitoloDiUscitaPrevisioneResponse response = capitoloUscitaPrevisioneService.aggiornaMassivoCapitoloDiUscitaPrevisione(request);
	...
	public void validate() {
		...
		if(isCategoriaCapitoloStandard(cup.getCategoriaCapitolo())) {
	 ...
	 */
	private boolean controllaDatiCapitoli(RicercaDettaglioMassivaCapitoloUscitaPrevisioneResponse response) {
		boolean almenoUnCapitoloInStatoValido = false;
		for(CapitoloUscitaPrevisione capitolo : response.getCapitoloMassivaUscitaPrevisione().getElencoCapitoli()) {
	 ...
	 */
	private boolean valutaVariazioniCapitolo(CapitoloUscitaPrevisione capitolo) {
