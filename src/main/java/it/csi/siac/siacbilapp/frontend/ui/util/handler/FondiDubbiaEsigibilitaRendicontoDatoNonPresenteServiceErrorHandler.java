...
/**
 * Handler per gli errori nei fondi a dubbia esigibilit&agrave;, rendiconto: modifica il messaggio di dato non presente
 * @author Marchino Alessandro
 *
 */
public class FondiDubbiaEsigibilitaRendicontoDatoNonPresenteServiceErrorHandler implements ServiceErrorHandler {
	...
	private List<Errore> handleErrori(List<Errore> errori) {
		...
		for(Errore err : errori) {
			if(ErroreCore.ENTITA_NON_TROVATA.getCodice().equals(err.getCodice())) {
				res.add(ErroreBil.ERRORE_GENERICO.getErrore("Capitolo entrata gestione: non e' presente in archivio o e' gia' collegato al fondo"));
			} else {
				res.add(err);
			}
		}
