 ...
 */
public abstract class PagedDataTableAjaxAction<S extends Serializable, M extends PagedDataTableAjaxModel<S>, 
	...
	public String execute() throws Exception {
		...
		if(!risCall.equals(SUCCESS)) {
			...
			for(int k = fromIndex; k < toIndex && k < totrec; k++) {
				S instance = getInstance(listaRisultati.get(k));
				handleAzioniConsentite(
						instance, 
						daRientro,
			...
			//SIAC-8005
			if(isAggiornareRientroPosizioneStartConsultazione()) {
				log.debug(methodName, "Salvo startPosition = " + startPosition);
				sessionHandler.setParametro(BilSessionParameter.RIENTRO_POSIZIONE_START_CONSULTAZIONE, startPosition);
			}
			//TODO per futura implementazione in caso di necessita'
//			if(isAggiornareRientroPosizioneStartAggiorna()) {
//				log.debug(methodName, "Salvo startPosition = " + startPosition);
//				sessionHandler.setParametro(BilSessionParameter.RIENTRO_POSIZIONE_START_AGGIORNAMENTO, startPosition);
//			}
			//
	...
	private void impostaRequestClass() {
		Class<?>[] genericTypeArguments = GenericTypeResolver.resolveTypeArguments(this.getClass(), PagedDataTableAjaxAction.class);
	...
	/**
	 * SIAC-8005
	 * Se sia da aggiornare la posizione di start per il rientro nella consultazione per gestire una nuova paginazione 
	 * @return <code>true</code> se la posizione &eacute; da aggiornare; <code>false</code> altrimenti
	 */
	 ...
	 * @return <code>true</code> se la posizione &eacute; da aggiornare; <code>false</code> altrimenti
	 */
	protected boolean isAggiornareRientroPosizioneStartAggiorna() {
		return false;
	}
