...
import it.csi.siac.siacgsaapp.frontend.ui.model.primanotaintegrata.ordinativo.incasso.CompletaOrdinativoIncassoInsPrimaNotaIntegrataGSAModel;

/**
 * Classe di action per l'inserimento della prima integrata, sezione dei movimenti dettaglio, ordinativo incasso. Modulo GSA
 * 
...
@PutModelInSession(GestioneOrdinativoIncassoInsPrimaNotaIntegrataBaseAction.MODEL_SESSION_NAME_COMPLETA_INS_ORDINATIVO_INCASSO_GSA)
public class CompletaOrdinativoIncassoInsContoPrimaNotaIntegrataGSAAction extends GestioneOrdinativoIncassoInsContoPrimaNotaIntegrataBaseAction <CompletaOrdinativoIncassoInsPrimaNotaIntegrataGSAModel> {

	/** Per la serializzazione */
	private static final long serialVersionUID = 1795683497776874085L;
	...
	/**
	 * Ottiene la lista di wrapper dei conti da visualizzare.
	 * @return la lista
	 * */
	private List<ElementoScritturaPrimaNotaIntegrata> ottieniListaElementoScritturaPNI() {
		...
		for(ContoTipoOperazione ctop : model.getCausaleEP().getContiTipoOperazione()){
			if(ctop.getClasseDiConciliazione() != null){
				RicercaContiConciliazionePerClasse req = model.creaRequestRicercaContiConciliazionePerConciliazione(ctop.getClasseDiConciliazione());
				...
				//controllare, forse ce lo ho gia'
