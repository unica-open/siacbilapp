...
/**
 * Classe base di action per l'inserimento della prima integrata, sezione dei movimenti dettaglio, ordinativi di incasso.
 * 
 * @author Marchino Alessandro
 * @version 1.0.0 - 12/10/2015
 * @param <M> la tipizzazione del model
 */
public abstract class GestioneOrdinativoIncassoInsPrimaNotaIntegrataBaseAction<M extends GestioneOrdinativoIncassoPrimaNotaIntegrataBaseModel>
		extends BaseInserisciAggiornaPrimaNotaIntegrataBaseAction<OrdinativoIncasso, ConsultaRegistrazioneMovFinOrdinativoIncassoHelper, M>  {
	...
	public static final String MODEL_SESSION_NAME_COMPLETA_INS_ORDINATIVO_INCASSO_FIN = "CompletaOrdinativoIncassoInsPrimaNotaIntegrataFINModel";
	/** Nome del model del completamento e validazione della reg con nuova primanota x la sessione */
	public static final String MODEL_SESSION_NAME_COMPLETA_INS_ORDINATIVO_INCASSO_GSA = "CompletaOrdinativoIncassoInsPrimaNotaIntegrataGSAModel";
	/** Nome del model del completamento della reg con nuova primanota per la sessione */
	public static final String MODEL_SESSION_NAME_COMPLETA_VALIDA_INS_ORDINATIVO_INCASSO_FIN = "CompletaValidaOrdinativoIncassoInsPrimaNotaIntegrataFINModel";
	/** Nome del model del completamento della reg con nuova primanota per la sessione */
	public static final String MODEL_SESSION_NAME_COMPLETA_VALIDA_INS_ORDINATIVO_INCASSO_GSA = "CompletaValidaOrdinativoIncassoInsPrimaNotaIntegrataGSAModel";
	...
	public String execute() throws Exception {
		...
		sessionHandler.setParametro(BilSessionParameter.ORDINATIVO_INCASSO, null);
	...
	private void getDatiDaOrdinativo(Ordinativo ordinativo){
		// Carico da servizio
		RicercaOrdinativoIncassoPerChiave req = model.creaRequestRicercaOrdinativoIncassoPerChiave(ordinativo);
		...
		OrdinativoIncasso ordinativoServizio = res.getOrdinativoIncasso();
		List<SubOrdinativoIncasso> listaSubOrdinativo = defaultingList(ordinativoServizio.getElencoSubOrdinativiDiIncasso());
		
		sessionHandler.setParametro(BilSessionParameter.ORDINATIVO_INCASSO, ordinativoServizio);
		
		model.setOrdinativoIncasso(ordinativoServizio);
		model.setClassificatoreGerarchico(model.getOrdinativoIncasso().getCapitoloEntrataGestione().getCategoriaTipologiaTitolo());
		
		model.setConsultazioneHelper(new ConsultaRegistrazioneMovFinOrdinativoIncassoHelper(ordinativoServizio, listaSubOrdinativo, model.isGestioneUEB()));
	...
	protected void computaStringheDescrizioneDaRegistrazione(RegistrazioneMovFin registrazioneMovFin){
		...
		descrizione.append("Ord Incasso ")
	...
	protected ElementoPianoDeiConti ottieniElementoPianoDeiContiDaMovimento() {
		return model.getOrdinativoIncasso() != null
			&& model.getOrdinativoIncasso().getCapitoloEntrataGestione() != null
				? model.getOrdinativoIncasso().getCapitoloEntrataGestione().getElementoPianoDeiConti()
				: null;
	}
	
	@Override
	protected Soggetto ottieniSoggettoDaMovimento() {
		return model.getOrdinativoIncasso() != null ? model.getOrdinativoIncasso().getSoggetto() : null;
	}
}
