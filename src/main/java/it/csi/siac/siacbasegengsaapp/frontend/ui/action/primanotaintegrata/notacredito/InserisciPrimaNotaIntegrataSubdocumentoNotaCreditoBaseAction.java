...
/**
 * Classe base di action per l'inserimento della prima integrata per il subdocumento tramite nota di credito.
 * 
 ...
 *
 */
public abstract class InserisciPrimaNotaIntegrataSubdocumentoNotaCreditoBaseAction <D extends Documento<S, SI>, S extends Subdocumento<D, SI>, SI extends SubdocumentoIva<D, S, SI>,
	E extends ElementoQuotaRegistrazioneMovFin<D, S>, H extends ConsultaRegistrazioneMovFinDocumentoHelper<D, S, SI>, M extends InserisciPrimaNotaIntegrataNotaCreditoBaseModel<D, S, SI, E, D, S, SI, H>>
		extends InserisciPrimaNotaIntegrataNotaCreditoBaseAction<D, S, SI, E, H, M> {
	...
	protected void impostaImportiScritture(List<ElementoScritturaPrimaNotaIntegrata> listaElementoScrittura) {
		...
		if(model.getSubdocumento().getSubdocumentoIva() != null) {
			...
			calcolaDatiScrittureIva(listaElementoScrittura, model.getSubdocumento().getSubdocumentoIva());
			return;
		}
		...
		calcolaDatiScritturaNoIvaRilevante(listaElementoScrittura, wrapper);
	}
	 ...
	 */
	private void calcolaDatiScrittureIva(List<ElementoScritturaPrimaNotaIntegrata> listaElementoScrittura, SI subdocumentoIva) {
		BigDecimal imponibile = subdocumentoIva.getTotaleImponibileMovimentiIva();
		BigDecimal imposta = subdocumentoIva.getTotaleImpostaMovimentiIva();
		...
		for(ElementoScritturaPrimaNotaIntegrata elementoScrittura : listaElementoScrittura) {
			setImportoInElementoScrittura(imponibile, imposta, elementoScrittura);
		}
	}
	 ...
	 * @param imposta           l'imposta
	 * @param elementoScrittura il wrapper della scrittura
	 */
	protected abstract void setImportoInElementoScrittura(BigDecimal imponibile, BigDecimal imposta, ElementoScritturaPrimaNotaIntegrata elementoScrittura);
	 ...
	 * @param wrapper                il wrapper
	 */
	private void calcolaDatiScritturaNoIvaRilevante(List<ElementoScritturaPrimaNotaIntegrata> listaElementoScrittura, E wrapper) {
		for(ElementoScritturaPrimaNotaIntegrata elementoScrittura : listaElementoScrittura) {
			setImportoInElementoScrittura(wrapper.getImponibileDaImpostare(), wrapper.getImpostaDaImpostare(), elementoScrittura);
		}
	}
...
//		elementoScrittura.getContoTipoOperazione().setConto(model.getContoDaSostituire());
	 ...
	 */
	private E findWrapperByUidSubdocumento() {
