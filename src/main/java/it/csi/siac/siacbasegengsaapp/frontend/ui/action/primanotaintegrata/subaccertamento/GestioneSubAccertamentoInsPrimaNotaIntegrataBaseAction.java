...
*/
package it.csi.siac.siacbasegengsaapp.frontend.ui.action.primanotaintegrata.subaccertamento;
...
import it.csi.siac.siacbasegengsaapp.frontend.ui.model.primanotaintegrata.subaccertamento.GestioneSubAccertamentoPrimaNotaIntegrataBaseModel;
...
public abstract class GestioneSubAccertamentoInsPrimaNotaIntegrataBaseAction<M extends GestioneSubAccertamentoPrimaNotaIntegrataBaseModel>
		extends BaseInserisciAggiornaPrimaNotaIntegrataBaseAction<Accertamento, ConsultaRegistrazioneMovFinSubAccertamentoHelper, M>  {

	/** Per la serializzazione */
	private static final long serialVersionUID = 1500519209241677461L;
	...
	public String execute() throws Exception {
		...
		sessionHandler.setParametro(BilSessionParameter.SUBACCERTAMENTO, null);
	...
	private void getDatiDaSubAccertamento(SubAccertamento subAccertamento){
		SubAccertamento subAccertamentoSessione = sessionHandler.getParametro(BilSessionParameter.SUBACCERTAMENTO);
		Accertamento accertamentoSessione = sessionHandler.getParametro(BilSessionParameter.ACCERTAMENTO);
		...
		if(subAccertamentoSessione == null || subAccertamentoSessione.getUid() != subAccertamento.getUid()) {
			...
			// Imposto in sessione
			sessionHandler.setParametro(BilSessionParameter.SUBACCERTAMENTO, subAccertamentoSessione);
			sessionHandler.setParametro(BilSessionParameter.ACCERTAMENTO, accertamentoSessione);
		}
		...
		// SIAC-5294
		model.setConsultazioneHelper(new ConsultaRegistrazioneMovFinSubAccertamentoHelper(accertamentoSessione, subAccertamentoSessione, model.isGestioneUEB()));
	}
	...
	protected void computaStringheDescrizioneDaRegistrazione(RegistrazioneMovFin registrazioneMovFin){
		...
		descrizione.append("SubAcc ")
			.append(subAccertamento.getAnnoAccertamentoPadre())
			.append("/")
