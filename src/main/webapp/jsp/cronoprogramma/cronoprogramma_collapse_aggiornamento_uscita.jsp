...
<div class="accordion_info">
	<fieldset id="fieldsetDettaglioUscita" class="form-horizontal">
		...
		<s:hidden id="HIDDEN_cronoUid" name="dettaglioUscitaCronoprogramma.uid" />
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<s:textfield id="numeroCapitoloUscita" cssClass="span2 soloNumeri" name="dettaglioUscitaCronoprogramma.numeroCapitolo" placeholder="%{'capitolo'}" maxlength="9"
					readonly="dettaglioUscitaFromEsistente" />
				...
				<span class="alRight">
					...
					<label class="radio inline">
						<input type="radio" value="si" id="daEsistenteUscitaSi" name="daEsistenteUscita" <s:if test="dettaglioUscitaFromEsistente">checked</s:if> />
						s&iacute;
					</label>
					<label class="radio inline">
						<input type="radio" value="no" id="daEsistenteUscitaNo" name="daEsistenteUscita" <s:if test="!dettaglioUscitaFromEsistente">checked</s:if> />
						no
					</label>
				</span>
		...
		<div class="control-group">
			...
			<div class="controls">
				<s:select id="missioneUscita" list="listaMissione" name="dettaglioUscitaCronoprogramma.missione.uid" cssClass="span10"
					...
					disabled="%{dettaglioUscitaCronoprogramma.numeroArticolo != null}" />
				<s:hidden id="HIDDEN_missioneUscita" name="dettaglioUscitaCronoprogramma.missione.uid" disabled="%{dettaglioUscitaCronoprogramma.numeroArticolo == null}" />
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<s:select id="programmaUscita" list="listaProgramma" name="dettaglioUscitaCronoprogramma.programma.uid" cssClass="span10"
					required="required" listKey="uid" listValue="%{codice + '-' + descrizione}" headerKey="" headerValue="Seleziona il programma"
					disabled="%{dettaglioUscitaCronoprogramma.numeroArticolo != null}" />
				<s:hidden id="HIDDEN_programmaUscita" name="dettaglioUscitaCronoprogramma.programma.uid" disabled="%{dettaglioUscitaCronoprogramma.numeroArticolo == null}" />
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<s:select id="titoloSpesaUscita" list="listaTitoloSpesa" name="dettaglioUscitaCronoprogramma.titoloSpesa.uid" cssClass="span10"
					...
					disabled="%{dettaglioUscitaCronoprogramma.numeroArticolo != null}" />
				<s:hidden id="HIDDEN_titoloSpesaUscita" name="dettaglioUscitaCronoprogramma.titoloSpesa.uid" disabled="%{dettaglioUscitaCronoprogramma.numeroArticolo == null}" />
			</div>
		</div>
	</fieldset>

	<p>
		<a data-close-collapse="#collapseDettaglioUscita" class="btn btn-secondary" href="#">annulla aggiornamento</a>
		<a data-url-fragment="AggiornaDettaglioCronoprogrammaUscita" data-close-collapse="#collapseDettaglioUscita" class="btn btn-secondary" href="#" data-save-button>
