...
;(function($, w, doc) {
    ...
    function ottieniQuoteIva() {
        ...
        $.postJSON("aggiornamentoDocumentoSpesa_ottieniQuoteRilevantiIva.do", function(data) {
    ...
    function popolaTabellaQuoteRilevantiIva(lista) {
        var opts = {
            ...
            aoColumnDefs: [
                ...
                {aTargets: [1], mData: "movimento"},
                {aTargets: [2], mData: "capitolo"},
                {aTargets: [3], mData: "attivitaIva"},
                {aTargets: [4], mData: "registrazioneIva"},
                ...
                {aTargets: [6], mData: function(source) {
                    return source.importoTotaleMovimentiIva.formatMoney();
                }, fnCreatedCell: function(nTd) {
                    $(nTd).addClass("tab_Right");
                }},
                {aTargets: [7], mData: function(source) {
                    ...
                    $("a", nTd).substituteHandler("click", function(e) {
                        var url = "aggiornamentoDocumentoSpesa_redirezione$DocumentoIvaSpesa_quota.do";
    ...
    function gestioneVisualizzazioneDiv(e) {
        ...
        if(selectedRadio.val() === "Documento" && legameConIvaPresente && !documentoIvaLegatoDocumentoPresente) {
            // Ho un documento iva legato ad una quota: impedisco l'apertura e la selezione del documento iva legato alle quote
    ...
    /**
     * Valutazione dell'apertura del tab dell'IVA.
     *
     * Il tab non puo' essere aperto se lo stato del documento non e' completo. (JIRA 2369)
     *
     * @param e (Event) l'evento scatenante l'invocazione
     */
    function valutaAperturaTabIva(e) {
        if(!isDocumentoCompleto()) {
            e.preventDefault();
            e.stopPropagation();
            bootboxAlert("FIN_ERR_0292 - Il documento non e' in stato completo. Le operazioni sull'IVA non sono consentite.");
        }
    }

    /**
     * Controlla se il documento sia completo.
     *
     * @returns (Boolean) true se il documento e' completo; false altrimenti
     */
    function isDocumentoCompleto() {
        return "true" === $("#HIDDEN_statoOperativoDocumentoCompleto").val();
    }
    ...
    $(function() {
        ...
        // Gestione dell'apertura del tab
        $("a[href='#tabDatiIva']", 'ul.nav-tabs').click(valutaAperturaTabIva);
    });
