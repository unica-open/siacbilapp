...
;(function($) {
    ...
    function dettaglioQuote(td, e, uid) {
        ...
        e.preventDefault();
        $.postJSON("risultatiRicercaTestataDocumentoSpesaDettaglioQuote.do", {uidDaConsultare : uid})
        .then(function(data) {
     ...
     */
    function popolaTabellaQuote(idTabella, lista) {
        var isOverlay = false;
        var table = $("#tabellaDettaglioQuoteDocumentoSpesa").overlay({rebind: true, loader: true, usePosition: true});
        var opts = {
            bPaginate: true,
            bLengthChange: false,
            iDisplayLength: 5,
    ...
    function visualizzaTabellaDocumenti() {
        var options = {
            ...
            sAjaxSource: "risultatiRicercaTestataDocumentoSpesaAjax.do",
