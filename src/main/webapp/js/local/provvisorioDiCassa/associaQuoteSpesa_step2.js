...
;(function($) {
    ...
    function populateTable(url) {
        ...
        var opts = {
            bServerSide: true,
            sAjaxSource: "risultatiRicercaQuoteSpesaDaAssociareAProvvisorioAjax.do",
            sServerMethod: "POST",
            bPaginate: true,
            bLengthChange: false,
            iDisplayLength: 50,
     ...
     */
    function submitForm() {
        var str = '<input type="hidden" name="totaleSubdocumentiSpesa" value="' + $('#totaleSubdocumentiSpesa').val() + '"/>';
        str = Object
            .keys(checkedData)
    ...
    function ricalcolaTotali() {
        ...
        var totaleSpesa = new BigNumber(0);
        Object
            .keys(checkedData)
            .map(function(key) {
            ...
            .forEach(function(el) {
                ...
                totaleSpesa = totaleSpesa.plus(importo).sub(importoDaDedurre);
            });
        
        $("#spanTotaleSubdocumentiSpesa").html(formatMoney(totaleSpesa));
        // SIAC-6060: verificare se sia possibile scrivere il dato in maniera differente
        $("#totaleSubdocumentiSpesa").val(totaleSpesa.toNumber());
        
        // SIAC-6077
        $('#spanImportoDaRegolarizzare').html(formatMoney(importoDaRegolarizzare.sub(totaleSpesa)));
    }
