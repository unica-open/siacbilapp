...
;(function($, global) {
    ...
    var ricercheModale = {};
    var tipiRicercheModale = ['Capitolo', 'Impegno', 'Soggetto', 'Provvedimento'];
    ...
    function associaImputazioniContabili(param, modal) {
    	$.postJSON('risultatiRicercaPreDocumentoSpesaAssocia.do', param).then(function(data){
    ...
    /**
     * Definisce la predisposizione di pagamento per i preDocumenti.
     *
     * @param param (Object) il parametro per la chiamata asincrona
     * @param modal (jQuery) il modale da chiudere
     */
    function definisciPredisposizionePagamento(param, modal) {
    	$.postJSON('risultatiRicercaPreDocumentoSpesaDefinisci.do', param).then(function(data){
    		//impostoErrorineglialert se ci sono errori
    		if(impostaDatiNegliAlert(data.errori, alertErrori)) {
    			return $.Deferred().reject().promise();
            }
     ...
     *
     * @returns (jQuery) l'oggetto jQuery rappresentante i preDocumenti selezionati
     */
     ...
     */
    function lavoraConSelezionati(e, suffissoTipo, msg, suffissoSpan, callback) {
     ...
     * @param callback     (Function) la funzione da invocare alla conferma
     */
    function lavoraConTutti(e, suffissoTipo, suffissoSpan, callback) {
        var modale = $('#modale' + suffissoTipo);
        var oggettoPerInvocazioneAsincrona = {};
    ...
    function openModificaImputazioniContabili(modale) {
        ...
        $.post('risultatiRicercaPreDocumentoSpesa_apriAssociaConModifiche.do')
        .then(function(data) {
            var idCapitolo;
            var idProvvedimento;
            ...
            idCapitolo = Capitolo.inizializza('UscitaGestione', '#annoCapitolo', '#numeroCapitolo', '#numeroArticolo', '#numeroUEB', '#datiRiferimentoCapitoloSpan', '#pulsanteCompilazioneGuidataCapitolo');
            idProvvedimento = Provvedimento.inizializzazione(Ztree, {}, 'AttoAmministrativo');
            Impegno.inizializza('#annoMovimentoMovimentoGestione', '#numeroMovimentoGestione', '#numeroSubMovimentoGestione', '#datiRiferimentoImpegnoSpan', undefined, undefined, undefined, undefined, '#pulsanteCompilazioneGuidataMovimentoGestione');
    ...
    function handleCodiceSoggetto() {
        var codiceSoggetto = $('#codiceSoggettoSoggetto');
        var sedeSecondaria = $('#accordionSedeSecondariaSoggetto');
        var modalitaPagamento = $('#accordionModalitaPagamentoSoggetto');
        var descrizioneSoggetto = $('#datiRiferimentoSoggettoSpan');
        $(codiceSoggetto).substituteHandler('change', Soggetto.caricaDettaglioSoggetto.bind(undefined, codiceSoggetto, sedeSecondaria, modalitaPagamento, descrizioneSoggetto));
        // Timeout per rendere visibile il campo e impedire un errato dimensionamento dell'overlay
        setTimeout(function() {
            codiceSoggetto.change();
        }, 50);
    }
    ...
    function visualizzaTabellaDocumenti() {
        ...
        var options = {
            bServerSide: true,
            sAjaxSource: 'risultatiRicercaPreDocumentoSpesaAjax.do',
            sServerMethod: 'POST',
            bPaginate: true,
            bLengthChange: false,
            iDisplayLength: 50,
            ...
            aoColumnDefs: [
                ...
                {aTargets: [4], mData: function(source) {
                    return source.contoTesoreria
                        ? source.contoTesoreria.codice + ' - ' + source.contoTesoreria.descrizione
                        : '';
                }},
                ...
                {aTargets: [7], mData: defaultPerDataTable('soggetto')},
                {aTargets: [8], mData: defaultPerDataTable('statoOperativoPreDocumento.codice')},
                {aTargets: [9], mData: defaultPerDataTable('dataDocumento', '', formatDate)},
                {aTargets: [10], mData: defaultPerDataTable('importo', 0, formatMoney), fnCreatedCell: function(nTd) {
                    $(nTd).addClass('tab_Right');
                }},
                {aTargets: [11], mData: 'azioni', fnCreatedCell: function(nTd, sData, oData) {
    ...
    /**
     * Definisce la predisposizione di pagamento per i preDocumenti selezionati.
     *
     * @param e (Event) l'evento scatenante l'invocazione
     */
    function definisciPredisposizionePagamentoSelezionati(e) {
        lavoraConSelezionati(e, 'Definisci', 'definire', 'Definire', definisciPredisposizionePagamento);
    }

    /**
     * Definisce la predisposizione di pagamento per tutti i preDocumenti.
     *
     * @param e (Event) l'evento scatenante l'invocazione
     */
    function definisciPredisposizionePagamentoTutti(e) {
        lavoraConTutti(e, 'Definisci', 'Definire', definisciPredisposizionePagamento);
    }
    ...
    function associaImputazioniContabiliSelezionati(e) {
        lavoraConSelezionati(e, 'AssociaImputazioniContabili', 'associare', 'Associare', associaImputazioniContabili);
    }
    ...
    function associaImputazioniContabiliTutti(e) {
        lavoraConTutti(e, 'AssociaImputazioniContabili', 'Associare', associaImputazioniContabili);
    }
    ...
    function inserisciPreDocumento() {
        document.location = 'risultatiRicercaPreDocumentoSpesaInserisci.do';
    }
    ...
    function modificaImputazioniContabili() {
        ...
        $.postJSON('risultatiRicercaPreDocumentoSpesa_associaConModifiche.do', qualify(oggettoPerInvocazioneAsincrona)).then(function(data){
    		//impostoErrorineglialert se ci sono errori
    		if(impostaDatiNegliAlert(data.errori, alertErrori)) {
    			return $.Deferred().reject().promise();
            }
    ...
    $(function() {
        ...
        $('#pulsanteDefinisciTutti').click(definisciPredisposizionePagamentoTutti);
        $('#pulsanteDefinisciSelezionati').click(definisciPredisposizionePagamentoSelezionati);
