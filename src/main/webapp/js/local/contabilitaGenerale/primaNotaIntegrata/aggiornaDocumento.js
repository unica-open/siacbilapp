...
var PrimaNota = function($,w){
    ...
    function caricaElencoScritture(e) {
         ...
         $.postJSON("aggiornaContoPrimaNotaIntegrataDocumentoFIN_ottieniListaConti.do", {uidMovimentoEPPerScritture: uidMovimentoEP})
         .then(function(data) {
             if(impostaDatiNegliAlert(data.errori, $('#ERRORI'))) {
...
//        alertInformazioni.slideUp();
        ...
        $.postJSON("aggiornaContoPrimaNotaIntegrataDocumentoFIN_aggiornaConto.do", obj)
        .then(function(data) {
    ...
    function eliminazioneConto(index) {
        ...
        $.postJSON("aggiornaContoPrimaNotaIntegrataDocumentoFIN_eliminaConto.do", {indiceConto: index})
        .then(function(data) {
            if(impostaDatiNegliAlert(data.errori, $('#ERRORI'))) {
    ...
    function salvaDatiQuote(param){
        var $elencoScrittureQuota = $("#elencoScrittureQuota").overlay('show');
    	$.postJSON("aggiornaContoPrimaNotaIntegrataDocumentoFIN_salvaDatiQuota.do", param)
         .then(function(data) {
         ...
         }).always($elencoScrittureQuota.overlay.bind($elencoScrittureQuota, 'hide'));
    }
    ...
    function gestioneRichiestaProsecuzione(messaggi) {
        ...
        bootboxAlert(msg, 'Attenzione', 'dialogWarn', [
            ...
            // Forzo il submit
            {label: 'S&igrave;, prosegui', 'class': 'btn', callback: salvaDatiQuote.bind(undefined, {proseguiImportiNonCongruenti: true})}
        ]);
    }
    ...
    function inserisciConto() {
        ...
        $.postJSON("aggiornaContoPrimaNotaIntegrataDocumentoFIN_inserisciConto.do", obj)
        .then(function(data) {
            if(impostaDatiNegliAlert(data.errori, alertErrori)) {
                // Errore: esco
                return;
